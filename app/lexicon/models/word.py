...
class Base(models.Model):
    ...
    word = models.ForeignKey(
        Word, unique=False, on_delete=models.CASCADE, related_name="base_set"
    )
    ...
    def krl_slugify(self, string):
        return "".join(
            [
                i
                for i in string.lower().replace("ü", "y")
                if i in self.get_krl_abc().lower()
            ]
        )
...
class WordForm(models.Model):
    ...
    word = models.ForeignKey(
        Word, unique=False, on_delete=models.CASCADE, related_name="form_set"
    )
    form = models.ForeignKey(
        GrammarForm, unique=False, on_delete=models.CASCADE, related_name="form"
    )
